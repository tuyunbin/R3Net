...
parser.add_argument('--input_captions_json', default='data/change_captions.json', help='input captions json file to process into hdf5')
parser.add_argument('--input_neg_captions_json', default='data/no_change_captions.json', help='input neg_captions json file to process into hdf5')
parser.add_argument('--input_image_dir', default='data/images', help='input image directory')
parser.add_argument('--input_vocab_json', default='')
parser.add_argument('--split_json', default='data/splits.json', help='json file that contains the dataset splits')
parser.add_argument('--output_vocab_json', default='data/vocab.json', help='output vocab file')
parser.add_argument('--output_h5', default='data/labels.h5', help='output h5 file')
...
def main(args):
    ...
    train_neg_captions = []
    train_captions_tag = []
    train_neg_captions_tag = []
    for img in train_imgs:
        cap = captions[img]
        neg_cap = neg_captions[img]
        train_captions.extend(cap)
        train_neg_captions.extend(neg_cap)
        train_captions_tag.append((img, cap))
        train_neg_captions_tag.append((img, neg_cap))
    ...
    # encode tag
    key_tags = ["green", "moved", "change", "metal", "location", "metallic",
                "cylinder", "modified", "shiny", "placed", "large", "difference",
                "matte", "rubber", "nothing", "purple", "identical", "cube", "missing",
                "brown", "appeared", "gone", "newly", "left", "sphere", "right",
                "remains", "turned", "big", "ball", "cyan", "made", "added", "changed",
                "became", "no", "occurred", "small", "blue", "object", "disappeared",
                "tiny", "gray", "different", "same", "yellow", "block", "red",
                "longer", "there"]

    ixtotag = {}
    tagtoix = {}

    for idx in range(len(key_tags)):
        tagtoix[key_tags[idx]] = idx
        ixtotag[idx] = key_tags[idx]

    train_label = np.zeros((len(all_imgs), 50))
    ...
    with h5py.File(args.output_h5, 'w') as f:
        ...
        f.create_dataset('tags', data=train_label)
        ...
        f.create_dataset('neg_tags', data=train_neg_label)
