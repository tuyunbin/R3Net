...
from models.modules_R3NET import ChangeDetectorDoubleAttDyn, AddSpatialInfo
...
label_criterion = TagLoss().to(device)
...
while t < cfg.train.max_iter:
    ...
    for i, batch in enumerate(train_loader):
        ...
        d_img_paths, nsc_img_paths, sc_img_paths, tag_feats, neg_tag_feats = batch
        ...
        tag_feats, neg_tag_feats = tag_feats.to(device), neg_tag_feats.to(device)
        d_feats, nsc_feats, sc_feats = \
        ...
        chg_pos_feat_bef, chg_pos_feat_aft, chg_pos_feat_diff, tag_feat, tag_pred = change_detector(d_feats, sc_feats)
        chg_neg_logits, chg_neg_att_bef, chg_neg_att_aft, \
        chg_neg_feat_bef, chg_neg_feat_aft, chg_neg_feat_diff, neg_tag_feat, neg_tag_pred = change_detector(d_feats, nsc_feats)
        ...
        # pos_dynamic_atts = speaker.get_module_weights()

        speaker_output_neg = speaker._forward(chg_neg_feat_bef,
                                              chg_neg_feat_aft,
                                              chg_neg_feat_diff,
                                              no_chg_labels, neg_tag_feat)
        # neg_dynamic_atts = speaker.get_module_weights()
        ...
        tag_loss = 0.5 * label_criterion(tag_pred, tag_feats) + \
                    0.5 * label_criterion(neg_tag_pred, neg_tag_feats)

        entropy_loss = -args.entropy_weight * entropy_criterion(dynamic_atts, masks[:, 1:])
        # entropy_loss = -args.entropy_weight * entropy_criterion(pos_dynamic_atts, masks[:, 1:]) + \
        #                -args.entropy_weight * entropy_criterion(neg_dynamic_atts, masks[:, 1:])
        # att_sum = (chg_pos_att_bef.sum() + chg_pos_att_aft.sum()+chg_neg_att_bef.sum() + chg_neg_att_aft.sum()) / (4 * batch_size)
        att_sum = (chg_pos_att_bef.sum() + chg_pos_att_aft.sum()) / (2 * batch_size)
        total_loss = speaker_loss + 2.5e-03 * att_sum + entropy_loss + 0.1 * tag_loss
        ...
        if t % cfg.train.snapshot_interval == 0:
            ...
            with torch.no_grad():
                ...
                for val_i, val_batch in enumerate(val_loader):
                    d_feats, nsc_feats, sc_feats, \
                    labels, no_chg_labels, masks, no_chg_masks, aux_labels_pos, aux_labels_neg, \
                    d_img_paths, nsc_img_paths, sc_img_paths, tag_feats, neg_tag_feats = val_batch
                    ...
                    speaker_output_neg, _ = speaker._sample(chg_neg_feat_bef,
                                                            chg_neg_feat_aft,
                                                            chg_neg_feat_diff,
                                                            no_chg_labels, neg_tag_feat, cfg)
