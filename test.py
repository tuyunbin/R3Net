...
prob_output_path = os.path.join(test_output_dir, 'probs', 'test')
if not os.path.exists(att_output_path):
    os.makedirs(att_output_path)

if not os.path.exists(prob_output_path):
    os.makedirs(prob_output_path)
...
with torch.no_grad():
    ...
    for i, batch in tqdm(enumerate(test_loader)):
        ...
        chg_pos_feat_bef, chg_pos_feat_aft, chg_pos_feat_diff, tag_feat, tag_prob = change_detector(d_feats, sc_feats)
        chg_neg_logits, chg_neg_att_bef, chg_neg_att_aft, \
        chg_neg_feat_bef, chg_neg_feat_aft, chg_neg_feat_diff, neg_tag_feat, neg_tag_prob = change_detector(d_feats, nsc_feats)


        speaker_output_pos, _ = speaker._sample(chg_pos_feat_bef,
                                                chg_pos_feat_aft,
                                                chg_pos_feat_diff,
                                                labels, tag_feat, cfg, sample_max=1)
        ...
        speaker_output_neg, _ = speaker._sample(chg_neg_feat_bef,
                                                chg_neg_feat_aft,
                                                chg_neg_feat_diff,
                                                no_chg_labels, neg_tag_feat, cfg, sample_max=1)
        ...
        tag_prob = tag_prob.cpu().numpy()
        neg_tag_prob = neg_tag_prob.cpu().numpy()
        ...
        for j in range(batch_size):
            ...
            tag_prob_path = os.path.join(prob_output_path, image_num)
            np.savetxt(tag_prob_path+'.txt', tag_prob[j], fmt='%0.8f')
            # with open(tag_prob_path, 'w') as f:
            #      numpy.savetxt(tag_prob[j], f)
